...
const PeopleFilter = ({
  ...
  return (
    <div className="flex flex-col w-full h-auto px-4 mb-24 md:mb-10 bg-tertiary-100 rounded-3xl overflow-y-scroll">
      ...
      <form>
        ...
        <div className="flex w-full h-auto mt-16">
          <span className="text-xs font-medium text-primary-500">
            Característcias del compañero
          </span>
        </div>
        <div className="flex flex-row flex-wrap w-full h-auto mt-4 justify-center items-center gap-4">
          {peopleDetails?.map((data) => {
            return (
              <FilterChip
                text={data.text}
                icon={data.icon}
